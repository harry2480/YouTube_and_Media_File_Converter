...
> 3. [ImageMagick](https://github.com/ImageMagick/ImageMagick)はOSSとしてGithubに公開されています。

> **【整理済み】ImageMagick 関連ファイル**
>
> ImageMagick の実行ファイルと設定ファイル（XML 等）は `ImageMagick` フォルダにまとめてあります。`magick.exe` をそのフォルダに置くか、システムの `PATH` に追加してください。
...
## ⚠️ 注意: ファイル名に特殊文字が含まれる場合の制約

- 一部の環境では、ファイル名やフォルダ名に `?`、`!`、`【】`、`&`、全角記号などの**特殊文字**が含まれていると、`ffmpeg` がファイルを正しく開けずに「Illegal byte sequence」などのエラーで変換に失敗する場合があります。
- 対処方法:
  1. 可能ならファイル名を英数字と `-_ .` のみを使った名前に変更してください（例: `iPhone_SE4.mp4`）。
  2. または、ファイルを英数字のみのパス（例: `C:\Temp`）に移動してから、バッチにドラッグ＆ドロップしてください。
  3. バッチは短いパス名（8.3形式）を自動的に試行しますが、システムで8.3名が無効な場合はリネームが必要になります。
